...
for t in range(100):
  ...
  #Main change for 4iii) is the following piece of code that maximises the change in the x value at 0.05
  x_diff1 = abs(x[0] - x_old1)
  if skip_first_loop>0.5:
    if x_diff1 <= 0.05:
      if x[0] > x_old1:
        x[0] = x_old1+0.05
      if x[0] > x_old1:
        x[0] = x_old1-0.05
  x_diff2 = abs(x[1] - x_old2)
  if skip_first_loop>0.5:
    if x_diff2 <= 0.05:
      if x[1] > x_old2:
        x[1] = x_old2+0.05
...
plt.savefig('V N star againt Time')
plt.cla()
plt.clf()
